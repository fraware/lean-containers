# Mathlib container API audit

Audit of how `lean-containers` relates to Mathlib 4 on the Lean 4.31 line. Use this document when
deciding whether to depend on this package, Mathlib, or both, and when preparing upstream
contributions.
...
## 1. Does Mathlib already have polynomial functor infrastructure?

**Yes — a mature univariate stack, plus multivariate and QPF material.**
...
## 2. Does Mathlib already have W-type API nearby?
...
## 3. Should containers live under CategoryTheory, Logic, or remain external?
...
existing `PFunctor` / `WType` files. This package should **remain external** (mathlib-free) until
maintainers agree on duplication risk and naming. See `docs/upstream/mathlib-issue-draft.md` for a
copy-paste issue template.

---

## 4. Which local simp lemmas are genuinely upstreamable?
...
Mathlib's simp set unless tied to an equivalence `Poly sig α ≃ P α`.
...
**What could be upstreamable later (with maintainer sign-off):**

- Small **examples** (lists, trees, branching transition systems) adjacent to `PFunctor` / `WType`.
- A thin **`Poly sig α ≃ P α`** equivalence, after which structure-based simp lemmas could be
  derived rather than duplicated.
- **Category-theoretic** initial-algebra material, only after the data-layer API is settled.

**Conclusion:** Keep `Poly.map_*` and `W.fold_sup` local until (a) an equivalence to `PFunctor` is
accepted in Mathlib, and (b) maintainers confirm desired simp normal form.
...
| Related docs | Summary: [`docs/mathlib-overlap.md`](../mathlib-overlap.md); issue template: [`docs/upstream/mathlib-issue-draft.md`](mathlib-issue-draft.md) |
